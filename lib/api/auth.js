...
/* =======================================================
   🧷 COMPAT-LAAG voor oud gebruik in AuthProvider
   - zodat bestaande imports blijven werken
======================================================= */

// 🔁 Oud: setCurrentUserId(id) → nu: schrijf id in hetzelfde object
export function setCurrentUserId(id: number | string) {
  if (typeof window === "undefined") return;

  const current = loadUserLocal() || {};
  const merged = { ...current, id: Number(id) };
  saveUserLocal(merged);
}

// 🔁 Oud: clearCurrentUserId() → nu: zelfde als clearUserLocal()
export function clearCurrentUserId() {
  clearUserLocal();
}

/* =======================================================
   🌐 Global fetch wrapper voor auth-routes
======================================================= */
async function fetchAuth(path: string, options: any = {}) {
  return await fetch(`${API_BASE_URL}${path}`, {
    ...options,
    credentials: "include", // ⬅️ kritische fix: cookies meesturen
    headers: {
...
export async function apiLogin(email: string, password: string) {
  try {
    ...
    // ✅ user (incl. id) opslaan voor apiClient/getCurrentUserId
    saveUserLocal(user);
