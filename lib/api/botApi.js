...
export const createManualOrder = async ({
  ...
  if (!quantity) throw new Error("quantity is verplicht");
  if (!price) throw new Error("price is verplicht");
...
/* =====================================================
   📈 11. PORTFOLIO BALANCE HISTORY (Chart)
===================================================== */
export const fetchPortfolioBalanceHistory = async ({
  ...
  const data = await fetchAuth(
    `/api/portfolio/balance-history?bucket=${bucket}&limit=${limit}`,
    { method: "GET" }
  );
...
/* =====================================================
   📈 12. PORTFOLIO BALANCE HISTORY (Chart) PER BOT
===================================================== */
export const fetchBotBalanceHistory = async ({
  ...
  const data = await fetchAuth(
    `/api/bot/balance-history?bot_id=${bot_id}&bucket=${bucket}&limit=${limit}`,
    { method: "GET" }
  );
