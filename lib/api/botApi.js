...
/* =====================================================
   💰 7. UPDATE BOT BUDGET (NIEUW)
===================================================== */

export const updateBotBudget = async (
  bot_id,
  {
    budget_total_eur = null,
    budget_daily_limit_eur = null,
    budget_min_order_eur = null,
    budget_max_order_eur = null,
  }
) => {
  if (!bot_id) throw new Error("bot_id is verplicht");

  return await fetchAuth(`/api/bot/${bot_id}/budget`, {
    method: "PUT",
    body: JSON.stringify({
      ...(budget_total_eur != null ? { budget_total_eur } : {}),
      ...(budget_daily_limit_eur != null
        ? { budget_daily_limit_eur }
        : {}),
      ...(budget_min_order_eur != null
