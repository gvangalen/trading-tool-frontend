...
export const createBotConfig = async ({
  ...
  if (!name) throw new Error("Bot name is verplicht");
  if (!bot_type) throw new Error("bot_type is verplicht");
...
export const updateBotConfig = async (bot_id, payload) => {
  ...
  if (!payload.bot_type) throw new Error("bot_type is verplicht");
