...
// ✅ 9. Haal forward return data op (alleen BTC), gegroepeerd per periode
export const fetchForwardReturns = async () => {
  try {
    const rawData = await fetchWithRetry(`${API_BASE_URL}/api/market_data/forward`, 'GET');
    ...
    rawData.forEach((item) => {
      const { period, year, returns } = item;
      if (grouped[period]) {
        grouped[period].push({
          year,
          values: returns,
    ...
    console.log('✅ Gegroepeerde forward return data:', grouped);
    return grouped;
  } catch (err) {
    console.error('❌ Fout bij ophalen of verwerken forward returns:', err);
    return null;
  }
};

export const fetchStructuredForwardReturns = () =>
  fetchWithRetry(`${API_BASE_URL}/api/market_data/forward/structured`, 'GET');
