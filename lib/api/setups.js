...
/*  
⭐ 9. NIEUW — laatste setup ophalen
*/
export const fetchLastSetup = async () => {
  try {
    const url = `${API_BASE_URL}/api/setups/last`;
    console.log(`🔍 [fetchLastSetup] GET: ${url}`);

    const response = await fetchWithRetry(url, 'GET');

    // Backend returnt: { setup: {...} }
    if (!response || !response.setup) {
      return null;
    }

    return response.setup;   // <-- BELANGRIJK
  } catch (error) {
    console.error('❌ [fetchLastSetup] Fout:', error);
    toast.error('Kon laatste setup niet laden.');
    return null;
  }
};

/*  
🔥 10. Actieve setup van vandaag ophalen (via Setup Agent)
*/
export const fetchActiveSetup = async () => {
  try {
    const url = `${API_BASE_URL}/api/setups/active`;
    console.log(`🔍 [fetchActiveSetup] GET: ${url}`);

    const res = await fetchWithRetry(url, 'GET');

    // API geeft terug: { active: {...} } of { active: null }
    if (!res || res.active === null) {
      return null;
    }

    return res.active;
  } catch (error) {
    console.error('❌ [fetchActiveSetup] Fout:', error);
    toast.error('Kon actieve setup niet laden.');
    return null;
  }
};
