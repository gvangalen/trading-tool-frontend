...
import { fetchAuth as fetchWithAuth } from '@/lib/auth/apiClient';   // ✅ JUISTE IMPORT
...
export const createStrategy = async (strategyData) => {
  ...
  return await fetchWithAuth(`/api/strategies`, 'POST', strategyData);
};
...
export const fetchStrategies = async (symbol = '', timeframe = '', tag = '') => {
  const res = await fetchWithAuth(`/api/strategies/query`, 'POST', {
    symbol,
    timeframe,
    tag,
  });
...
export const updateStrategy = async (id, data) =>
  await fetchWithAuth(`/api/strategies/${id}`, 'PUT', data);
...
export const deleteStrategy = async (id) =>
  await fetchWithAuth(`/api/strategies/${id}`, 'DELETE');
...
export const generateStrategy = async (setupId, overwrite = true) =>
  await fetchWithAuth(`/api/strategies/generate/${setupId}`, 'POST', {
    overwrite,
  });
...
export const generateAllStrategies = async () =>
  await fetchWithAuth(`/api/strategies/generate_all`, 'POST');
...
export const fetchStrategySummary = async () =>
  await fetchWithAuth(`/api/strategies/summary`, 'GET');
...
export const fetchScoreMatrix = async () =>
  await fetchWithAuth(`/api/strategies/score_matrix`, 'GET');
...
export const fetchStrategyExplanation = async (id) =>
  await fetchWithAuth(`/api/strategies/${id}/explanation`, 'GET');
...
export const exportStrategiesCSV = async () =>
  await fetchWithAuth(`/api/strategies/export`, 'GET');
...
export const fetchLastStrategy = async () => {
  const res = await fetchWithAuth(`/api/strategies/last`, 'GET');
...
//
// =========================================================
// 12. STRATEGIE PER SETUP (BELANGRIJK VOOR AI-REFRESH)
// =========================================================
...
export const fetchTaskStatus = async (taskId) =>
  await fetchWithAuth(`/api/tasks/${taskId}`, 'GET');
