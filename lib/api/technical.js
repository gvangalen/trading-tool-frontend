...
//
// 🧠 ➕ RULES VOOR SCORELOGICA EN ZOEKVELD
//

// 📡 [getIndicatorNames] Haal alle indicatornamen op
export const getIndicatorNames = async () => {
  console.log('📡 [getIndicatorNames] Ophalen van /api/technical/indicators');
  const data = await fetchWithRetry(`${API_BASE_URL}/api/technical/indicators`, 'GET');  // ✅ aangepast
  console.log('📥 [getIndicatorNames] Gegevens ontvangen:', data);
  return data;
};

// 📡 [getScoreRulesForIndicator] Haal ALLE regels op (filter zelf in frontend)
export const getScoreRulesForIndicator = async (indicatorName) => {
  console.log(`📡 [getScoreRulesForIndicator] Ophalen van ALLE regels`);
  const allRules = await fetchWithRetry(`${API_BASE_URL}/api/technical_indicator_rules`, 'GET'); // ✅ aangepast
  console.log('📥 [getScoreRulesForIndicator] Alles ontvangen:', allRules);

  // Filter client-side op de juiste indicator
