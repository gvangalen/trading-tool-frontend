...
//
// ➕ 2. Voeg nieuwe technische indicator toe (alleen naam nodig)
//
...
export const technicalDataDelete = async (indicator) => {
  ...
  const data = await fetchWithRetry(`${API_BASE_URL}/api/technical_data/${indicator}`, 'DELETE');
  console.log("✅ [technicalDataDelete] Response:", data);
  return data;
};

// ✅ Alias voor consistent gebruik (frontend kan deleteTechnicalIndicator() blijven gebruiken)
export const deleteTechnicalIndicator = technicalDataDelete;

//
// 🔍 4. Filter technische data op asset (optioneel)
//
export const technicalDataBySymbol = async (symbol) => {
  console.log(`🔍 [technicalDataBySymbol] Ophalen van data voor ${symbol}`);
...
// 📡 Haal alle beschikbare indicatornamen op (voor dropdown)
export const getIndicatorNames = async () => {
  console.log('📡 [getIndicatorNames] Ophalen van /api/technical/indicator_names');
  const data = await fetchWithRetry(`${API_BASE_URL}/api/technical/indicator_names`, 'GET');
  console.log('📥 [getIndicatorNames] Gegevens ontvangen:', data);
  return data;
};

// 📡 Haal scoreregels op voor één specifieke indicator
export const getScoreRulesForIndicator = async (indicatorName) => {
  ...
  const data = await fetchWithRetry(`${API_BASE_URL}/api/technical_indicator_rules/${indicatorName}`, 'GET');
  console.log('📥 [getScoreRulesForIndicator] Regels ontvangen:', data);
  return data;
};
