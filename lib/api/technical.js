...
export const getIndicatorNames = async () => {
  console.log('📡 [getIndicatorNames] Ophalen van /api/indicators/technical');
  const data = await fetchWithRetry(`${API_BASE_URL}/api/indicators/technical`, 'GET');
  console.log('📥 [getIndicatorNames] Gegevens ontvangen:', data);
  return data;
};

export const getScoreRulesForIndicator = async (indicatorName) => {
  console.log(`📡 [getScoreRulesForIndicator] Ophalen van rules voor ${indicatorName}`);
  const data = await fetchWithRetry(`${API_BASE_URL}/api/technical_indicator_rules/${indicatorName}`, 'GET');
  console.log('📥 [getScoreRulesForIndicator] Gegevens ontvangen:', data);
  return data;
};
