...
export const generateExplanation = async (input) => {
  try {
    return await fetchWithRetry(`${API_BASE_URL}/api/setup/explanation`, 'POST', input);
  } catch (error) {
...
/*  
🔍 7. Check of setupnaam al bestaat voor symbool
*/
export const checkSetupNameExists = async (name, symbol = 'BTC') => {
  try {
    const res = await fetchWithRetry(
      `${API_BASE_URL}/api/setups/check_name?name=${encodeURIComponent(name)}&symbol=${encodeURIComponent(symbol)}`,
      'GET'
    );
